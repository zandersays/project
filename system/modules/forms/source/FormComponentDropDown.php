<?php
class FormComponentDropDown extends FormComponent {
    ...
    function __construct($id, $label, $dropDownOptionArray, $optionArray = array()) {
        ...
        $this->id = $id;
        $this->name = $this->id;
        $this->class = 'formComponentDropDown';
        $this->label = $label;
    ...
    public static function getStateArray($selectedState = null, $continentalOnly = false) {
        ...
        $continentalArray = array(array('value' => '', 'label'  => 'Select a State', 'disabled' => true), array('value' => 'AL', 'label'  => 'Alabama'), array('value' => 'AZ', 'label'  => 'Arizona'), array('value' => 'AR', 'label'  => 'Arkansas'), array('value' => 'CA', 'label'  => 'California'), array('value' => 'CO', 'label'  => 'Colorado'), array('value' => 'CT', 'label'  => 'Connecticut'), array('value' => 'DE', 'label'  => 'Delaware'), array('value' => 'DC', 'label'  => 'District of Columbia'), array('value' => 'FL', 'label'  => 'Florida'), array('value' => 'GA', 'label'  => 'Georgia'), array('value' => 'ID', 'label'  => 'Idaho'), array('value' => 'IL', 'label'  => 'Illinois'), array('value' => 'IN', 'label'  => 'Indiana'), array('value' => 'IA', 'label'  => 'Iowa'), array('value' => 'KS', 'label'  => 'Kansas'), array('value' => 'KY', 'label'  => 'Kentucky'), array('value' => 'LA', 'label'  => 'Louisiana'), array('value' => 'ME', 'label'  => 'Maine'), array('value' => 'MD', 'label'  => 'Maryland'), array('value' => 'MA', 'label'  => 'Massachusetts'), array('value' => 'MI', 'label'  => 'Michigan'), array('value' => 'MN', 'label'  => 'Minnesota'), array('value' => 'MS', 'label'  => 'Mississippi'), array('value' => 'MO', 'label'  => 'Missouri'), array('value' => 'MT', 'label'  => 'Montana'), array('value' => 'NE', 'label'  => 'Nebraska'), array('value' => 'NV', 'label'  => 'Nevada'), array('value' => 'NH', 'label'  => 'New Hampshire'), array('value' => 'NJ', 'label'  => 'New Jersey'), array('value' => 'NM', 'label'  => 'New Mexico'), array('value' => 'NY', 'label'  => 'New York'), array('value' => 'NC', 'label'  => 'North Carolina'), array('value' => 'ND', 'label'  => 'North Dakota'), array('value' => 'OH', 'label'  => 'Ohio'), array('value' => 'OK', 'label'  => 'Oklahoma'), array('value' => 'OR', 'label'  => 'Oregon'), array('value' => 'PA', 'label'  => 'Pennsylvania'), array('value' => 'RI', 'label'  => 'Rhode Island'), array('value' => 'SC', 'label'  => 'South Carolina'), array('value' => 'SD', 'label'  => 'South Dakota'), array('value' => 'TN', 'label'  => 'Tennessee'), array('value' => 'TX', 'label'  => 'Texas'), array('value' => 'UT', 'label'  => 'Utah'), array('value' => 'VT', 'label'  => 'Vermont'), array('value' => 'VA', 'label'  => 'Virginia'), array('value' => 'WA', 'label'  => 'Washington'), array('value' => 'WV', 'label'  => 'West Virginia'), array('value' => 'WI', 'label'  => 'Wisconsin'), array('value' => 'WY', 'label'  => 'Wyoming'));
        if($continentalOnly){
            $stateArray = $continentalArray;
        }
    ...
    public static function getHourArray($minHour, $maxHour, $hourInterval = 1, $selectedHour = null) {
        $hourArray = array();
        for($i = $minHour; $i <= $maxHour; $i += $hourInterval) {
            $option = array('value' => $i, 'label' => date('g:i A', strtotime('today +'.($i*60).' minutes')));
            if($i == $selectedHour) {
    ...
    function __toString() {
        ...
        // Select tag
        $select = new HtmlElement('select', array(
            'id' => $this->id,
            'name' => $this->name,
            'class' => $this->class,
        ));
        ...
        if($this->size != null) {
            $select->attr('size', $this->size);
        }
        if($this->width != null) {
            $select->attr('style', 'width:'.$this->width);
        }
        ...
        foreach($optGroupArray as $optGroup) {
            ${$optGroup} = new HtmlElement('optgroup', array('label' => $optGroup));
        }
        ...
        foreach($this->dropDownOptionArray as $dropDownOption) {
            if(isset($dropDownOption['optGroup']) && !empty($dropDownOption['optGroup'])) {
                ...
                if($this->initialValue !== null && $this->initialValue == $dropDownOption['value']) {
                    $optionSelected = true;
                }
            
                ${$dropDownOption['optGroup']}->append($this->getOption($optionValue, $optionLabel, $optionSelected, $optionDisabled));
            }
        }
        ...
        foreach($this->dropDownOptionArray as $dropDownOption) {
            ...
            if(isset($dropDownOption['optGroup']) && !empty($dropDownOption['optGroup']) && !isset(${$dropDownOption['optGroup'].'Added'})) {
                $select->append(${$dropDownOption['optGroup']});
            ...
            // Add any other elements
                ...
                $select->append($this->getOption($optionValue, $optionLabel, $optionSelected, $optionDisabled));
            }
        }
