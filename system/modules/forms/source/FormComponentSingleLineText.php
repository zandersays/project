...
class FormComponentSingleLineText extends FormComponent {
    var $subLabel;
    ...
    function __construct($id, $label, $optionArray = array()) {
        ...
        $this->name = $this->id;
        $this->label = $label;
        $this->class = 'formComponentSingleLineText';
    ...
    function __toString() {
        ...
        if(!empty($this->width)) {
            if(array_key_exists($this->width, $this->widthArray)) {
                $input->attr('style', 'width: '.$this->widthArray[$this->width].';');
            }
            else {
                $input->attr('style', 'width: '.$this->width.';');
            }
        }
        if($this->initialValue !== '') {
            $input->attr('value', $this->initialValue);
        }
        ...
        if($this->enterSubmits) {
            $input->attr('class', ' formComponentEnterSubmits', true);
        }
    ...
    public function decimalNegative($options) {
        ...
        $isDecimal = $this->decimal($options['value']);
    ...
    public function integerNegative($options) {
        ...
        $value = intval($options['value']);
        return ($value < 0) ? 'success' : $messageArray;
    }
    ...
    public function matches($options) {
        $componentToMatch = $this->parentFormSection->parentFormPage->form->select($options['matches']);
    ...
    public function maxMoneyValue($options) {
        $messageArray = array('Must be numeric with a maximum value of ' . $options['maxMoneyValue'] . '.');
        $maxValue = preg_replace('/\$|\,/', '', $options['maxMoneyValue']);
        $userInput = preg_replace('/\$|\,/', '', $options['value']);
        return $maxValue >= $userInput ? 'success' : $messageArray;
    }

    public function minMoneyValue($options) {
        $messageArray = array('Must be numeric with a minimum value of ' . $options['minMoneyValue'] . '.');
        $minValue = preg_replace('/\$|\,/', '', $options['minMoneyValue']);
        $userInput = preg_replace('/\$|\,/', '', $options['value']);
        return $minValue <= $userInput ? 'success' : $messageArray;
    }
    ...
    public function money($options) {
        ...
        $options['value'] = preg_match('/,/', '', $options['value']);
        return preg_match('/^((-?\$)|(\$-?)|(-))?((\d+(\.\d{2})?)|(\.\d{2}))$/', $options['value']) || $options['value'] == '' ? 'success' : $messageArray;
    }
