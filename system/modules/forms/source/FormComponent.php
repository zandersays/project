...
abstract class FormComponent {
    ...
    var $value = null;
    var $style = null;
    var $parentFormSection;
    var $anonymous = false;

    // Label
    var $label = null;  // Must be implemented by child class
    var $labelClass = 'formComponentLabel';
    var $labelRequiredStarClass = 'formComponentLabelRequiredStar';

    // Helpers
    var $tip = null;
    var $tipClass = 'formComponentTip';
    var $description = null;
    var $descriptionClass = 'formComponentDescription';
    ...
    function setInitialValues($formComponentData) {
        // Handle component instances
        if(Arr::is($formComponentData) && Arr::hasKey(0, $formComponentData)) {
            // Handle instance values on components like multiple choice
            if(Object::methodExists('hasInstanceValues', $this)) {
                if($this->hasInstanceValues()) {
                    $this->instanceOptions['initialValues'] = $formComponentData;
                }
                else {
                    $this->initialValue = $formComponentData;
                }
            }
            else {
                $this->instanceOptions['initialValues'] = $formComponentData;
        ...
        // Handle normal instances
            ...
            if(Object::is($formComponentData)) {
                $this->initialValue = Object::arr($formComponentData);
            }
            else {
                $this->initialValue = $formComponentData;
            }
    ...
    function generateComponentDiv($includeLabel = true) {
        ...
        $componentDiv = new HtmlElement('div', array(
            'id' => $this->id.'-wrapper',
            'class' => 'formComponent '.$this->class,
        ));
        ...
        // Style
        if(!empty($this->style)) {
            $componentDiv->attr('style', $this->style, true);
        }

        // Label tag
        if($includeLabel) {
            $label = $this->generateComponentLabel();
            $componentDiv->append($label);
        }
    ...
    function generateComponentLabel() {
        ...
        $label = new HtmlElement('label', array(
            'id' => $this->id.'-label',
            'for' => $this->id,
        ...
        if(in_array('required', $this->validationOptions)) {
            ...
            $label->append($labelRequiredStarSpan);
        }
    ...
    function insertComponentTip($div) {
        ...
        if(!empty($this->tip)) {
            $tipDiv = new HtmlElement('div', array(
                'id' => $this->id.'-tip',
                'style' => 'display: none;',
                'class' => $this->tipClass,
            ));
            $tipDiv->html($this->tip);
            $div->append($tipDiv);
        }
